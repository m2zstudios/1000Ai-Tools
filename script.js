(() => {
  ...
  const state = {
    allTools: Array.isArray(window.tools) ? [...new Map(window.tools.map(tool => [tool.id, tool])).values()] : [],
    filtered: [],
    listLimit: 24,
    ...
    return `
      <article class="tool-card">
        <div class="tool-card-top">
          <img class="tool-logo" src="${logo}" alt="${escapeHtml(tool.title)} logo" loading="lazy" decoding="async" referrerpolicy="no-referrer" onerror="if(!this.dataset.fb){this.dataset.fb='1';this.src='./fallback.png';}else{this.style.display='none';}">
  ...
  function initIndexPage() {
    ...
    $("#trendingGrid").innerHTML = state.allTools.slice(0, 7).map(createCard).join("");
  ...
  function initDetailedPage() {
    ...
    article.innerHTML = `
      <div class="detail-head">
        <img class="tool-logo tool-logo-lg" src="${detailLogo}" alt="${escapeHtml(tool.title)} logo" loading="eager" decoding="async" referrerpolicy="no-referrer" onerror="if(!this.dataset.fb){this.dataset.fb='1';this.src='./fallback.png';}else{this.style.display='none';}">
        <div>
      ...
      <div class="card-actions">
        <button class="btn btn-secondary ${favoriteState ? "fav-inline-active" : ""}" id="favoriteDetailBtn">${favoriteState ? "★" : "☆"}</button>
